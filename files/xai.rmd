...
set.seed(123)
n_muestra <- 100
filas <- sample(nrow(df), n_muestra)
df_sub <- df[filas, ]
...
ggplot(rf.part, aes(x = hum, y = temp, fill = yhat)) +
  ...
  geom_rug(data = df_sub, aes(x = hum), sides = "b", alpha = 0.5, inherit.aes = FALSE) +  # abajo
  geom_rug(data = df_sub, aes(y = temp), sides = "l", alpha = 0.5, inherit.aes = FALSE) +    # izquierda
  theme_minimal() +
